...
async function fetchOpenMeteoWeather(params) {
  ...
  u.searchParams.set('current', 'temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,rain,showers,snowfall,weather_code,cloud_cover,surface_pressure,visibility,wind_speed_10m,wind_gusts_10m');
  u.searchParams.set('hourly', 'precipitation_probability,weather_code,temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m');
  u.searchParams.set('daily', 'weather_code,temperature_2m_max,temperature_2m_min,apparent_temperature_max,apparent_temperature_min,precipitation_probability_max,precipitation_sum,wind_speed_10m_max,sunrise,sunset,uv_index_max');
  u.searchParams.set('forecast_days', '7');
  u.searchParams.set('timezone', location.timezone || 'auto');
  const body = await requestJson(u.toString(), { headers: { 'User-Agent': UA } });
  const cur = body && body.current || {};
  const hourlyForecast = buildOpenMeteoHourlyForecast(body && body.hourly, cur.time);
  const dailyForecast = buildOpenMeteoDailyForecast(body && body.daily);
  const precipitationProbability = hourlyForecast.length ? hourlyForecast[0].precipitationProbability : NaN;
  const weather = {
    provider: 'open-meteo',
    location: {
      name: location.name,
    ...
    surfacePressure: Number(cur.surface_pressure),
...
function openMeteoArrayValue(data, key, index) {
  const arr = data && data[key];
  return Array.isArray(arr) ? arr[index] : undefined;
}

function openMeteoArrayNumber(data, key, index) {
  const value = openMeteoArrayValue(data, key, index);
  const n = Number(value);
  return Number.isFinite(n) ? n : null;
}

function buildOpenMeteoHourlyForecast(hourly, currentTime) {
  const times = hourly && Array.isArray(hourly.time) ? hourly.time : [];
  if (!times.length) return [];
  let start = 0;
  if (currentTime) {
    const idx = times.findIndex(time => String(time || '') >= String(currentTime));
    if (idx >= 0) start = idx;
  }
  return times.slice(start, start + 24).map((time, offset) => {
    const index = start + offset;
    const weatherCode = openMeteoArrayNumber(hourly, 'weather_code', index);
    return {
      time: time || '',
      label: openMeteoWeatherLabel(weatherCode),
      weatherCode,
      temperature: openMeteoArrayNumber(hourly, 'temperature_2m', index),
      apparentTemperature: openMeteoArrayNumber(hourly, 'apparent_temperature', index),
      humidity: openMeteoArrayNumber(hourly, 'relative_humidity_2m', index),
      windSpeed: openMeteoArrayNumber(hourly, 'wind_speed_10m', index),
      precipitationProbability: openMeteoArrayNumber(hourly, 'precipitation_probability', index),
    };
  });
}

function buildOpenMeteoDailyForecast(daily) {
  const times = daily && Array.isArray(daily.time) ? daily.time : [];
  if (!times.length) return [];
  return times.slice(0, 7).map((date, index) => {
    const weatherCode = openMeteoArrayNumber(daily, 'weather_code', index);
    return {
      date: date || '',
      label: openMeteoWeatherLabel(weatherCode),
      weatherCode,
      temperatureMax: openMeteoArrayNumber(daily, 'temperature_2m_max', index),
      temperatureMin: openMeteoArrayNumber(daily, 'temperature_2m_min', index),
      apparentTemperatureMax: openMeteoArrayNumber(daily, 'apparent_temperature_max', index),
      apparentTemperatureMin: openMeteoArrayNumber(daily, 'apparent_temperature_min', index),
      precipitationProbability: openMeteoArrayNumber(daily, 'precipitation_probability_max', index),
      precipitationSum: openMeteoArrayNumber(daily, 'precipitation_sum', index),
      windSpeedMax: openMeteoArrayNumber(daily, 'wind_speed_10m_max', index),
      sunrise: openMeteoArrayValue(daily, 'sunrise', index) || '',
      sunset: openMeteoArrayValue(daily, 'sunset', index) || '',
      uvIndexMax: openMeteoArrayNumber(daily, 'uv_index_max', index),
    };
  });
}
