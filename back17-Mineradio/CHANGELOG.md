...
- 基于上游 `v1.1.1` 源码制作，并保留 GPL-3.0、原作者版权和第三方声明。
- Home 左侧施工区域改为与原界面一致的天气预报面板。
- 天气面板支持点击进入详情，展示 8 项实时指标、未来 24 小时和未来 7 天预报。
- 新增城市搜索、定位、刷新和天气电台操作；主界面继续保持 6 小时与 4 天的紧凑预报。
- Open-Meteo 数据补充气压、能见度、阵风、降水总量、日出日落和 UV 指数。
- GitHub 发布与应用内更新地址切换为 `back17/Mineradio`。
